...
```{r}
## iirc wide form is more human readable and long form is more table readable. pawbly? Most Likely.
...
## one day i'll get used to how you can use dollar sign to select
```
...
filtered_d = d |>
  ...
  ## so is.na sets rows with its arg to NA, then filter from the dplyr package drops those rows.
  ## why do we not go filter for not donotuse? but i guess then we'd have to specify the value of dnu?
...
filtered_d = filtered_d |>
  select(c("Subject", "Cond"),                      # Generally important columns for both hypotheses
         ...
         starts_with("DinerDashWith"), c("SOFMusicEnemies", "SOFNoMusicEnemies")) 
          # These columns are for hypothesis 2

## so we're keeping subject & cond, and columns that contain game,
## but not the columns that contain intro, or which games, and also excluding comments,
## and also ignoring things that are for hypothesis 2.
## data cleaning is probably a lot easier if you know what data means rip.
```
...
## you know, i don't really understand how it knows to pivot only some columns and not others,
## but i'm going to be satisfied with the knowledge that people who programmed tidyverse were very smart
```
...
## so we don't hav to use quotes here... salute emoji
```
...
```{r}
rating_hyp_long_d_edit = rating_hyp_long_d |>
  mutate(
...
```{r}
rating_hyp_long_d_edit2 = rating_hyp_long_d_edit |>
  mutate(
...
```{r}
rating_summary_d = rating_hyp_long_d_edit2 |>
    group_by(Emotion, IsRecall) |>
    mutate(MeanRating = mean(Rating))

## i'm unsure what the summarize would do here? if i summarize immediately after mutating then
## it sends a warning about grouping by emotion & actually does not keep the new meanrating column
## which i doubt is intended?
## maybe i'll catch this on a resubmit?
## but for now i don't know if i need a summarize
## from the later code it also doesn't seem like they want a table of only a few vars?
...
performance_summary_d = performance_diff_d |>
  ...
  mutate(
    MeanImprovementScore = mean(PostMusic - PreMusic)
  )
  # your code here
```
...
```{r}
## standard error is the standard deviation divided by the root of sample size! 
## i think we have to put it as a value into the table...?
## so i think we mutate it for stdev!
## what is the standard error... umm...
## do i need to group-by this before i process it in...
## i didn't tell it to do row-wise, but it still only gives me 0 for the sd(MeanImprovementScore) ?? 

performance_summary_dsem = performance_summary_d |>
  group_by(ConfrontationalGame, MusicCondition) |>
  mutate(
    SEM = sd(MeanImprovementScore)
      ##sqrt(max(performance_summary_d$Subject))
  )
...
```{r}
SEM = sd(performance_summary_d$MeanImprovementScore) / sqrt((length(performance_summary_d)/2))
```
