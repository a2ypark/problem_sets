...
rating_summary_d = rating_hyp_long_d_edit2 |>
    group_by(Emotion, IsRecall, ConfrontationalGame) |>
...
#   )
  ## summarize keeps the groupby organization when it outputs the table, so it does better here
# 
# performance_summary_d2 = performance_diff_d |>
#   group_by(ConfrontationalGame, MusicCondition) |>
#   summarize(
#     MeanImprovementScore = mean(PostMusic - PreMusic),
#     SEM = sd(PostMusic - PreMusic)/sqrt(n())
#   )  
...
performance_summary_d2 = performance_diff_d |>
  ...
  ## added the SEM calculation inside this summarize to avoid having to do it again later for error bars
...
performance_summary_d2 |>
  ggplot(aes(x=ConfrontationalGame, y=MeanImprovementScore, fill=MusicCondition, group=MusicCondition)) +
  geom_bar(position="dodge", stat="identity") + 
  geom_errorbar(
    aes(x = ConfrontationalGame, ymin = (MeanImprovementScore - SEM), ymax = (MeanImprovementScore + SEM)),
